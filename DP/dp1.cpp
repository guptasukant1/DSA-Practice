...
// todo | The problem states that we have to find the number of ways to paint the fence with k colors.
// todo | We have to paint the fence with k colors such that no more than two adjacent fences have the same color.
// todo | 1st way to paint: last 2 posts are same. So, we have ans as (k - 1) * solve(n - 2, k). n - 2 will give us 2 remaining posts
// todo | Here, the n - 2 posts have set colours, and the last 2 are supposed to be same, and that will have k - 1 available colours.
// todo | 2nd way to paint: last 2 posts are different. So, we have ans as (k - 1) * solve(n - 1, k). n - 1 will give us 1 remaining post
// todo | Here, the n - 1 posts have set colours, but the last is supposed to be different, and that will have k - 1 available colours.
